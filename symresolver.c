...
struct symbol* symresolver_register_symbol(
    ...
    void* data)
{
    ...
    struct symbol* new_sym = calloc(1, sizeof(struct symbol));
    new_sym->name = sym_name;
    new_sym->data = data;
    new_sym->type = type;

    symresolver_push_symbol(process, new_sym);
    return new_sym;
}
...
{
    compiler_error(process, "Structs are not yet supported\n");
}
