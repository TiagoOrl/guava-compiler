...
static void expect_keyword(const char *keyword)
{
    ...
    if (!next_token || next_token->type != TOKEN_TYPE_KEYWORD || !S_EQ(next_token->sval, keyword))
        compiler_error(current_process, "Expecting the keyword %s", keyword);
}
...
static void expect_op(const char* op)
{
    ...
    if (!next_token || next_token->type != TOKEN_TYPE_OPERATOR || !S_EQ(next_token->sval, op))
        compiler_error(current_process, "Expecting the operator %s but something else was provided", next_token->sval);
...
void parser_reorder_expression(struct node** node_out)
{
    ...
    if (
        (is_array_node(node->exp.left) || is_node_assignment(node->exp.right)) ||
...
void parser_ignore_int(struct datatype* dtype)
{
    ...
    {
        compiler_error(current_process, "You have provided a secondary \"int\" type however its not supported with this current abbreviation\n");
    }
...
void parse_function(struct datatype* return_type, struct token* name_token, struct history* history)
{
    struct vector* arguments = NULL;
    parser_scope_new();

    make_function_node(return_type, name_token->sval, NULL, NULL);
    struct node* function_node = node_peek();

    parser_current_function = function_node;

    if (datatype_is_struct_or_union(return_type))
    {
        function_node->func.args.stack_addition += DATA_SIZE_DWORD;
    }

    expect_op("(");
    arguments = parse_function_arguments(history_begin(0));
    expect_sym(')');

    function_node->func.args.vector = arguments;
...
void parse_statement(struct history* history)
{
    ...
    // all statements ends with semicolons
    expect_sym(';');
}
...
void parser_append_size_for_variable_list(struct history* history, size_t* var_size, struct vector* vec)
{
    ...
    while(node)
    {
        parser_append_size_for_node(history, var_size, node);
        node = vector_peek_ptr(vec);
    }
}


void parser_append_size_for_node(struct history* history, size_t* var_size, struct node* node)
{
    if (!node)
        return;
    ...
    if (node->type == NODE_TYPE_VARIABLE)
    {
        ...
        {
            parser_append_size_for_node_struct_union(history, var_size, node);
            return;
        }

        *var_size += variable_size(node);
    }
    
    else if(node->type == NODE_TYPE_VARIABLE_LIST)
    {
        parser_append_size_for_variable_list(history, var_size, node->var_list.list);
    }
}


void parser_finalize_body(
    struct history* history, 
...
void parse_body_single_statement(size_t* variable_size, struct vector* body_vec, struct history* history)
{
    ...
    // change the variable_size by the size of the stmt_node
...
void parse_struct_or_union(struct datatype* dtype) 
{
    switch (dtype->type)
    {
        ...
        default:
            compiler_error(current_process, "COMPILER BUG: The provided datatype is not a struct or union\n");
    }
}
...
struct vector* parse_function_arguments(struct history* history)
{
    ...
    while(!token_next_is_symbol(')'))
    {
        ...
        struct node* arguments_node = node_pop();


        vector_push(arguments_vec, &arguments_node);
...
void parse_switch(struct history* history)
{
    ...
    struct node* switch_node_exp = node_pop();
    size_t var_size = 0;
    ...
    make_switch_node(switch_node_exp, body_node, _switch.case_data.cases, _switch.case_data.has_default_case);
