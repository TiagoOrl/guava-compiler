...
struct fixup_system* fixup_sys_new()
{
    ...
    system->vector_fixups = vector_create(sizeof(struct fixup));
...
{
    vector_set_peek_pointer(system->vector_fixups, 0);
}


struct fixup* fixup_next(struct fixup_system* system)
{
    return vector_peek_ptr(system->vector_fixups);
}
...
void fixup_sys_free(struct fixup_system* system)
{
    ...
    vector_free(system->vector_fixups);
    free(system);
}
...
struct fixup* fixup_register(struct fixup_system* system, struct fixup_config* config)
{
    ...
    fixup->system = system;
    vector_push(system->vector_fixups, fixup);
    return fixup;
}
